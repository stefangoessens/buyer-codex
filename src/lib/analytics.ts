...
export interface AnalyticsEventMap extends LaunchEventMap {
  ...
  /** Fired when the Chrome extension intake API resolves successfully. */
  extension_intake_succeeded: {
    platform: "zillow" | "redfin" | "realtor";
    outcome: "created" | "duplicate";
    authState: "signed_in" | "signed_out";
  };
  /** Fired when the Chrome extension intake API fails deterministically. */
...
export const EVENT_METADATA: Record<AnalyticsEventName, EventMetadata> = {
  ...
  extension_intake_succeeded: {
    category: "funnel",
    owner: "growth",
    whenFired: "Chrome extension intake API resolves to created or duplicate",
    piiSafe: true,
  },
  extension_intake_failed: {
    category: "system",
