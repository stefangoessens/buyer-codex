...
async function syncCatalogPromptsInDb(
  ...
  for (const entry of DEFAULT_PROMPT_REGISTRY_ENTRIES) {
    ...
    if (!activateMissing || !entry.isActive) continue;
...
export const getActiveVersionRefs = internalQuery({
  ...
  handler: async (ctx, args) => {
    const engineType = args.engineType;
...
export const getConsoleSnapshot = query({
  ...
  handler: async (ctx) => {
    ...
    const byEngine = new Map<
      PromptRegistryEngineType,
      Array<ReturnType<typeof toPromptDoc>>
    >();
...
export const registerPrompt = internalMutation({
  ...
  handler: async (ctx, args) => {
    const promptKey = normalizePromptKey(args.promptKey);
    const seededVersion = DEFAULT_PROMPT_REGISTRY_ENTRIES.find(
      (entry) =>
    ...
    const version = seededVersion ?? generateVersionHash(
...
export const activateVersion = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const promptKey = normalizePromptKey(args.promptKey);
    const target = await findPromptRow(
      ctx,
      args.engineType,
      promptKey,
      args.version,
